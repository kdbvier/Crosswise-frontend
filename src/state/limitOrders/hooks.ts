...
import { Currency, CurrencyAmount, TokenAmount, Trade, Token, Price, ETHER } from '@crosswise/sdk'
...
const getErrorMessage = (
  ...
  if (price) {
    if (rateType === Rate.MUL && (price.lessThan(trade.executionPrice) || price.equalTo(trade.executionPrice))) {
      return 'Only possible to place sell orders above market rate'
    }
    if (
      rateType === Rate.DIV &&
      (price.invert().greaterThan(trade.executionPrice.invert()) ||
        price.invert().equalTo(trade.executionPrice.invert()))
    ) {
...
const queryParametersToSwapState = (parsedQs: ParsedQs): OrderState => {
