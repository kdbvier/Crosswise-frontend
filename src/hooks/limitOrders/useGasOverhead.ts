...
import { CurrencyAmount, Price, Token, TokenAmount, JSBI, ETHER } from '@crosswise/sdk'
...
import getPriceForOneToken from 'views/OrderBook/utils/getPriceForOneToken'
...
export default function useGasOverhead(
  ...
  const bufferedOutputAmount = useMemo(() => {
    ...
    const margin: any = gasCostInInputTokens.outputAmount.multiply('2000').divide('10000')
  ...
  const realExecutionPrice = useMemo(() => {
    ...
    if (inputIsBNB && requiredGasAsCurrencyAmount.greaterThan(inputAmount)) return undefined
  ...
  const realExecutionPriceAsString = useMemo(() => {
    ...
    if (inputIsBNB && requiredGasAsCurrencyAmount.greaterThan(inputAmount)) {
      return 'never executes'
    }
    if (gasCostInInputTokens && gasCostInInputTokens.outputAmount.greaterThan(inputAmount)) {
      return 'never executes'
    }
