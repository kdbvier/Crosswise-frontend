import { ChainId, Currency, currencyEquals, JSBI, Price } from '@crosswise/sdk'
import { useMemo } from 'react'
import useActiveWeb3React from 'hooks/useActiveWeb3React'
import { BUSD, CRSS, WBNB } from '../config/constants/tokens'
...
export default function useBUSDPrice(currency?: Currency): Price | undefined {
  ...
  const tokenPairs: [Currency | undefined, Currency | undefined][] = useMemo(
    () => [
      ...
      [chainId ? WBNB[chainId] : undefined, chainId === ChainId.MAINNET ? BUSD_MAINNET : undefined],
    ],
  ...
  return useMemo(() => {
    ...
    if (wrapped.equals(WBNB[chainId])) {
      if (busdPair) {
        const price = busdPair.priceOf(WBNB[chainId])
    ...
    const ethPairETHBUSDValue: JSBI =
      ethPairETHAmount && busdEthPair ? busdEthPair.priceOf(WBNB[chainId]).quote(ethPairETHAmount).raw : JSBI.BigInt(0)
    ...
    if (ethPairState === PairState.EXISTS && ethPair && busdEthPairState === PairState.EXISTS && busdEthPair) {
      if (busdEthPair.reserveOf(BUSD_MAINNET).greaterThan('0') && ethPair.reserveOf(WBNB[chainId]).greaterThan('0')) {
